...
public:
    ...
    void Draw()
    {
        food.Draw();
        snake.Draw();
        DrawText(TextFormat("High Score: %i", highScore), offset + cellSize * cellCount - 200, 20, 20, darkGreen); // Draw high score
    }
    ...
    void CheckCollisionWithFood()
    {
        if (Vector2Equals(snake.body[0], food.position))
        {
            ...
            score++;
            if (score > highScore) // Update high score if current score exceeds it
            {
                highScore = score;
            }
            PlaySound(eatSound);
